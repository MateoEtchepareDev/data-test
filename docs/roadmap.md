...
- [x] `js/mapa.js` + `data/geo/provincias.json`: mapa por provincia (Leaflet), destaca la provincia con mayor volumen, click en provincia con datos muestra sus ventas en la leyenda; solo las provincias con ventas (del Excel) se sombrean en escala y muestran etiqueta permanente con el monto; provincias sin datos quedan como contorno tenue y no clicables (`pointer-events: none`).
